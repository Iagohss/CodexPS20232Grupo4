...
const ListaDeTarefas = () => {
  ...
  useEffect(() => {
    ...
    async function fetchTarefas() {
      const autentication = {
        usuarioEmail: context!.email,
        usuarioSenha: context!.senha,
      };
  ...
  const putTarefa = async (newTarefa: TTarefa) => {
    const dadosComSenha: DadosComSenha = {
      ...newTarefa,
      usuarioSenha: context!.senha,
    };
    const nTarefa = await doPUTtarefa(dadosComSenha);
    setTarefas(
      tarefas.map((tarefa) => (tarefa.id === nTarefa.id ? nTarefa : tarefa))
    );
  };
